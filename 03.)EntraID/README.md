...
## 7. Hands-On Implementation & PowerShell Automation

To operationalize Entra ID management in production, the following PowerShell automation scripts using the modern **Microsoft.Graph** SDK demonstrate identity provisioning, role-based access control (RBAC), and conditional access governance.

### 1. Connecting to Microsoft Graph SDK
```powershell
# Connect with required administrative scopes
Connect-MgGraph -Scopes "User.ReadWrite.All", "Group.ReadWrite.All", "RoleManagement.ReadWrite.Directory", "Policy.ReadWrite.ConditionalAccess"

# Verify connected tenant and admin context
Get-MgContext | Select-Object TenantId, Account, Scopes
```

### 2. Automated Security Group & Identity Provisioning
```powershell
# Create Security Group for Tier 1 Network Administrators
$GroupParams = @{
    DisplayName        = "SG-Network-Infrastructure-Admins"
    Description        = "Administrative group for enterprise network and cloud connectivity engineers"
    MailEnabled        = $false
    MailNickname       = "netadmins"
    SecurityEnabled    = $true
}
$NetAdminGroup = New-MgGroup @GroupParams

# Provision a Cloud-Only Network Administrator Identity
$PasswordProfile = @{
    ForceChangePasswordNextSignIn = $true
    Password                      = "InitialTempSecPass2026!"
}
$UserParams = @{
    AccountEnabled    = $true
    DisplayName       = "Philippe Truong (NetAdmin)"
    MailNickname      = "ptruong"
    UserPrincipalName = "ptruong.admin@corp.onmicrosoft.com"
    PasswordProfile   = $PasswordProfile
    UsageLocation     = "US"
}
$NewAdmin = New-MgUser @UserParams

# Add User to the Network Infrastructure Security Group
New-MgGroupMember -GroupId $NetAdminGroup.Id -DirectoryObjectId $NewAdmin.Id
```

### 3. Role-Based Access Control (RBAC) Assignment
Granting least-privilege network access to cloud virtual networks without granting global admin permissions:
```powershell
# Assign "Network Contributor" role at the Azure Subscription scope via Azure CLI
# Network Contributor allows managing VNets, subnets, NSGs, and VPN gateways without access to VMs or storage
az role assignment create \
    --assignee "ptruong.admin@corp.onmicrosoft.com" \
    --role "Network Contributor" \
    --scope "/subscriptions/<Subscription-ID>/resourceGroups/rg-enterprise-network-prod"
```

### 4. Automated Conditional Access Policy (Require MFA for All Admins)
```powershell
# Define Conditional Access Policy enforcing MFA on all administrative roles
$CAPolicy = @{
    DisplayName = "SEC-01-Enforce-MFA-For-Privileged-Roles"
    State       = "enabled"
    Conditions  = @{
        Users = @{
            IncludeRoles = @(
                ...
                "158830ec-f6d7-41f9-acb1-6c47bf50acd6"  # Network Administrator
            )
            ExcludeUsers = @("breakglass-admin@corp.onmicrosoft.com") # Emergency Break-Glass Exemption
        }
        Applications = @{
            IncludeApplications = @("All") # Applies to all cloud apps and Azure Portal
        }
        ClientAppTypes = @("all")
    }
    GrantControls = @{
        Operator     = "OR"
        BuiltInControls = @("mfa") # Enforces Multi-Factor Authentication
    }
}
...
### 5. On-Premises Entra Connect Sync Monitoring
When managing the hybrid bridge on Windows Server, use these commands to verify directory synchronization health:
```powershell
# Inspect the synchronization schedule (Default: every 30 minutes)
Get-ADSyncScheduler

# Manually trigger an immediate delta sync after adding users on-prem
Start-ADSyncSyncCycle -PolicyType Delta

# Verify the last sync status and connector run profiles
